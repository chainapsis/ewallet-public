...
describe("e2e_test_front_running_add_ed25519", () => {
  let ctx: TestContext;
  ...
  it("session hijacking: KSN register_ed25519 with different id_token than committed → INVALID_REQUEST", async () => {
  ...
  it("replay: KSN register_ed25519 called twice → API_ALREADY_CALLED (or 400)", async () => {
    const client = generateClientKeypair();
    const sessionId = generateSessionId();
  ...
  it("op/api mismatch: add_ed25519 session calling reshare → INVALID_REQUEST", async () => {
    const client = generateClientKeypair();
    const sessionId = generateSessionId();
    const idHash = computeIdTokenHash(AUTH_TYPE, TOKEN_A);
    const commit = await request(ctx.ksnApps[0])
  ...
  it("oko_api op/api mismatch: add_ed25519 session calling keygen (secp256k1) → INVALID_REQUEST", async () => {
    const client = generateClientKeypair();
    const sessionId = generateSessionId();
    const idHash = computeIdTokenHash(AUTH_TYPE, TOKEN_A);

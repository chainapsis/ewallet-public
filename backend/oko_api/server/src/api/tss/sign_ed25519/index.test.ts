...
describe("Ed25519 Signing", () => {
  ...
  describe("runSignEd25519Round1", () => {
    it("should generate commitments successfully", async () => {
      ...
      const result = await runSignEd25519Round1(pool, TEST_ENCRYPTION_SECRET, request);
  ...
  describe("runSignEd25519Round2", () => {
    ...
    it("should fail when using COMPLETED session for Round2", async () => {
      ...
      const round1Res = await runSignEd25519Round1(pool, TEST_ENCRYPTION_SECRET, {
        email: TEST_EMAIL,
        wallet_id: walletId,
        customer_id: customerId,
        msg: [...testMessage],
      ...
      const round2Res = await runSignEd25519Round2(pool, TEST_ENCRYPTION_SECRET, {
        email: TEST_EMAIL,
        wallet_id: walletId,

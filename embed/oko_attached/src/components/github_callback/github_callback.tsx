...
import { useGithubCallback } from "@oko-wallet-attached/components/github_callback/use_callback";
import { useSetThemeInCallback } from "@oko-wallet-attached/hooks/theme";

export const GithubCallback: FC = () => {
  const theme = useSetThemeInCallback("github");
...
const ErrorMessage: FC<{ error: string }> = ({ error }) => {
  ...
  const errorMessage = isParamsNotSufficient
    ? `Error Code: ${errorCode}\n\nFailed to get the required information from GitHub. Please contact Oko for support.`
    : `Error Code: ${errorCode}`;
